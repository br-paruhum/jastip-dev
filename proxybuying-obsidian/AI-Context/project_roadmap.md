...
- [x] **Task-28** - Update Request Reship work flow after Carrier arrived and Customs Duty was paid. - User prefer to do "Test, take notes, code update, next test" method when working on this task.
- [x] **Task-29** - ✅ DONE 02-Jul (`969550f`, `95ffcf6`, branch `task27-unread`; on STAGING). Proxy estimate now routes to the Buyer for Accept/Reject before it ever reaches the Carrier board, instead of jumping straight to "open for carrier offers" on Send Estimate. New `Status.ESTIMATE_SENT` sits between `OPEN` and `RESPONDED` (migration 0059, metadata-only). Buyer's Notes tab gains Accept/Reject buttons at this stage; Accept moves the order to the existing `RESPONDED` (open to Carriers, unchanged downstream); Reject cancels the order (already filterable via "Hide closed items" on both "My Orders" and "My Proxy Buying" — no template change needed there). Proxy's Notes tab gains an "Edit Estimate" button (same view now also handles edits while awaiting the buyer's decision). Buyer↔Proxy Message tab now opens as soon as the estimate is sent (was gated on deposit paid). New estimate_sent/estimate_accepted/estimate_rejected notification emails.
